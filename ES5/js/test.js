...
/**
 * @description 减法运算
 * @param {Array<string>} args 复制对象
 * @param {String} name 用户名
 * @param {Number} age 用户年龄
 * @param {Boolean} isVip 是否是vip
 * @param {Set<string>} children 孩子
 * @returns {Boolean} 是否是真
 */
function easyCopy (args, name, age, isVip, children) {
...
$(function () {
	...
	var area = {
		//上-下，极限
		up_down: {
			start: Math.tan(pi * 5 / 12),
			end: Math.tan(pi * 7 / 12)
		},
		//左-右，升
		left_right: {
			start: Math.tan(-pi / 12),
			end: Math.tan(pi / 12)
		},

		//左上 升
		left_up1: {
			start: Math.tan(pi * 3 / 12),
			end: Math.tan(pi * 5 / 12)
		},
		left_up2: {
			start: Math.tan(pi / 12),
			end: Math.tan(pi * 3 / 12)
		},
		//右上 升
		right_up1: {
			start: Math.tan(pi * 7 / 12),
			end: Math.tan(pi * 9 / 12)
		},
		right_up2: {
			start: Math.tan(pi * 9 / 12),
			end: Math.tan(pi * 11 / 12)
		},
	};
	//获取dom列表
	...
	function getDistance (domList) {
		...
		for (var i = 0; i < distantArray.length; i++) {
			distantArray[i] = {
				x: domList.eq(i).offset().left + 100,
				y: domList.eq(i).offset().top + 100
			}
		}
		return distantArray;
	}
